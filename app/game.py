...
class Game:
    ...
    def __init__(self, id: UUID, host: UUID, players: set[UUID], status: Status, game_state: GameState, created: datetime):
        self.id: UUID = id
        self.host: UUID = host
    ...
    def new_game(cls, host: UUID) -> 'Game':
        players = set()
        players.add(host)
        return cls(id=uuid4(), host=host, players=players, status=Status.NEW,
            game_state=GameState.new_game(host), created=datetime.now())
